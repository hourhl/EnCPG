package io.joern.autodetect4notrace
...
/**
  ...
  */
case class AutoDetect4NoTraceOpts(var outputPath: String = "")
    extends LayerCreatorOptions {}

class AutoDetect4NoTrace(options: AutoDetect4NoTraceOpts) extends LayerCreator {
  override val overlayName: String = AutoDetect4NoTrace.overlayName
  override val description: String = AutoDetect4NoTrace.description
  ...
  private class VulnerabilityDetectionPass(cpg: Cpg) extends CpgPass(cpg) {
    ...
    override def run(builder: DiffGraphBuilder): Unit = {
      println("[*] Starting vulnerability detection on methods...")
      ...
      val memcpy_vulnerabilities = methods.flatMap { method =>
      ...
      // 1.2 strncpy
      // Check for vulnerable copy function patterns
      println("[*] 1.2 copy functions (strncpy, memcpy, memmove) analysis")
      val cpy_vulnerabilities = methods.flatMap { method =>
      ...
      val offByOneVulnerabilities = detectOffByOne(methods)
      ...
      val nullPtrVulnerabilities = detectNullPointerDereference(methods)
      ...
      val fieldAccessVulnerabilities = detectFieldAccessWithoutNullCheck(methods)
      ...
      val useAfterFreeVulnerabilities = detectUseAfterFree(methods)
      ...
      val cmdExecVulnerabilities = detectCommandExecution(methods)
      ...
      val integerOverflowVulnerabilities = detectIntegerOverflow(methods)
      ...
      val reallocIntOverflowVulnerabilities = detectReallocIntegerOverflow(methods)
      ...
      if (options.outputPath.nonEmpty) {
        ...
        println("[!] No output path specified. Use opts.autodetect4notrace.outputPath = \"/path/to/VulReport.txt\"")
      }
    ...
    /**
      ...
      */
    private def detectNullPointerDereference(methods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, String, String, Int, String, Int)] = {
      val methodsfilter = methods.filterNot(_.name == "<global>")
      methodsfilter.flatMap { m =>
    ...
    /**
      * Detects command execution vulnerabilities in methods.
      ...
      */
    private def detectCommandExecution(methods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, String, Int, String, Int)] = {
      val methodsfilter = methods.filterNot(_.name == "<global>")
      methodsfilter.flatMap { method =>
    ...
    /**
      ...
      */
    private def detectFieldAccessWithoutNullCheck(methods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, Int, String, String)] = {
      val methodsfilter = methods.filterNot(_.name == "<global>")
      val allVulnerabilities = scala.collection.mutable.ListBuffer[(String, Int, String, String)]()
      
      for (method <- methodsfilter) {
    ...
    /**
      * Detects use-after-free vulnerabilities in methods.
      ...
      */
    private def detectUseAfterFree(methods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, String, Int, String, String, Int, String, Int, String, String)] = {
      val realMethods = methods.filterNot(_.name == "<global>")
      val results = realMethods.flatMap { method =>
    ...
    /**
      * Detects off-by-one vulnerabilities in methods.
      * Returns a list of tuples: (methodName, checkFuncName, checkLine, checkCode, writeFuncName, writeLine, writeCode)
      */
    private def detectOffByOne(methods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, String, Int, String, String, Int, String)] = {
      val methodsfilter = methods.filterNot(_.name == "<global>")
      ...
      val checkFunctionNames = methodsfilter.filter(looksLikeCheckFunction).map(_.name).toSet
      
      val results = methodsfilter.flatMap { m =>
    ...
    /**
      * Detects integer overflow vulnerabilities in methods.
      * Returns a list of tuples: (methodName, lineNumber, code)
      */
    private def detectIntegerOverflow(methods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, Int, String)] = {
      val methodsFiltered = methods.filterNot(_.name == "<global>")
      
      // 1. Find all suspicious assignments
      val suspiciousAssignments = methodsFiltered.flatMap { method =>
    ...
    /**
      ...
      */
    private def detectReallocIntegerOverflow(allMethods: List[io.shiftleft.codepropertygraph.generated.nodes.Method]): List[(String, Int, String, Int, String)] = {
      val allMethodsFilter = allMethods.filterNot(_.name == "<global>")
      
      allMethodsFilter.flatMap { method =>
